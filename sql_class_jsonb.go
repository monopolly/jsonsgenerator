...
func (a *SQL) updateJsonbMap(x *Field) []byte {
	...
	var list []string
	list = append(list, "\n//update sql query")
	list = append(list, fmt.Sprintf(`func (a *%s) Update%s(id any, k string, v any) (err error) {`, settings.SQL.Class, x.Go.Name))
...
func (a *SQL) deleteJsonbMapKey(x *Field) []byte {
	...
	list = append(list, fmt.Sprintf(`func (a *%s) DeleteKey%s(id any, k string) (err error) {`, settings.SQL.Class, x.Go.Name))
	...
	list = append(list, fmt.Sprintf(`_,err = conn.Exec(c, q, k, id)`))
	list = append(list, "return")
	list = append(list, "}")
	return []byte(strings.Join(list, "\n"))
}
...
func (a *SQL) updatesJsonbMap(x *Field) []byte {
	...
	list = append(list, fmt.Sprintf(`func (a *%s) Updates%s(id any, keys map[string]any) (err error) {`, settings.SQL.Class, x.Go.Name))
	...
	list = append(list, `b, _ := json.Marshal(keys) `)
...
func (a *SQL) updateJsonbArray(x *Field) []byte {
	...
	*/
	...
	list = append(list, fmt.Sprintf(`func (a *%s) Delete%s(id any, v any) (err error) {`, settings.SQL.Class, x.Go.Name))
