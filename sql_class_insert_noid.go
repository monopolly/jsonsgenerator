...
// go struct: insert sql query
func (a *SQL) insertNoIDNoConflict() []byte {

	var list []string
	list = append(list, "//Insert struct and no return id")
	list = append(list, fmt.Sprintf(`func (a *%s) InsertNoConflict(v *%s) (err error) {`, settings.SQL.Class, settings.Go.StructName))
	...
	q := `q := "insert into tableName (fieldsList) values (valuesList) on conflict do nothing"`
	q = tools.Replace(q, "tableName", settings.SQL.Table)
	q = tools.Replace(q, "fieldsList", strings.Join(flist, ", "))
	q = tools.Replace(q, "valuesList", strings.Join(vars, ", "))
